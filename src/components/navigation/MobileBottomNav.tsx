    ...
    <nav
      ...
      className={`fixed inset-x-0 bottom-0 z-40 backdrop-blur-2xl px-2 pt-1.5 transition-colors duration-200 ${
        isDark
          ...
          : 'bg-white/95 border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]'
      }`}
    >
        ...
        {TABS.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex h-12 min-w-0 flex-col items-center justify-center gap-0.5 rounded-2xl text-[10px] transition-all duration-200 active:scale-95 ${
                isSelected
                  ? isDark
                    ? 'bg-gradient-to-r from-[#FF2D55] to-[#FF4570] text-white shadow-md shadow-rose-500/25 scale-[1.02] font-black'
                    : 'bg-black text-white shadow-sm scale-[1.02] font-bold'
                  : isDark
                  ? 'text-slate-400 hover:text-white active:bg-slate-800/80 font-medium'
                  : 'text-slate-500 hover:text-slate-900 active:bg-slate-100/80 font-medium'
              }`}
            >
