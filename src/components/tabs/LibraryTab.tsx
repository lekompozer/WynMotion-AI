...
export const LibraryTab: React.FC<LibraryTabProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* ─── SOUND & MUSIC LIBRARY HERO BANNER ─── */}
      <div
        onClick={() => setIsMusicLibraryModalOpen(true)}
        className={`group relative rounded-[26px] p-4 sm:p-4.5 border cursor-pointer transition-all duration-300 active:scale-[0.99] overflow-hidden backdrop-blur-xl ${
          isDark
        ...
        <div className="relative z-10 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3.5 min-w-0">
            <div className="relative shrink-0">
              ...
              <div className="relative w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 flex items-center justify-center text-white shadow-md">
                <Music className="w-5 h-5 sm:w-6 sm:h-6 animate-pulse" />
              </div>
            </div>
            <div className="min-w-0">
              <h2 className={`text-sm sm:text-base font-black tracking-tight truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>
                Sound & Music Library
              </h2>
              <p className={`text-[11px] sm:text-xs font-medium mt-0.5 truncate ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                From WynMotion (No Copyright Music)
              </p>
            </div>
          </div>

          <div className="flex items-center justify-center shrink-0 w-8 h-8 rounded-full transition-transform group-hover:translate-x-1">
            <ChevronRight className={`w-5 h-5 ${isDark ? 'text-slate-400 group-hover:text-white' : 'text-slate-500 group-hover:text-slate-900'}`} />
          </div>
        </div>
      </div>
