...
export interface CapCutGalleryItem {
  ...
  authorAvatar: string;
  coverUrl: string;
  aspectClass: string; // so le aspect ratios
  badge?: string;
}
...
export const CapCutGalleryModal: React.FC<CapCutGalleryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#090A10] text-white animate-in fade-in duration-200">
      {/* ── TOP APP BAR ── */}
      <div className="pt-[max(env(safe-area-inset-top,44px),44px)] px-4 pb-2 border-b border-slate-800/80 bg-[#090A10]/95 backdrop-blur-md flex items-center gap-3">
        <button
          type="button"
          onClick={onClose}
